...
const ImageSlider = () => {
  ...
  return (
    <div className="flex px-2 gap-2 ">
      <div
        className="relative w-[100%] rounded-[18px] h-[20vh] lg:h-[55vh] bg-red-400"
      >
        {Accordions.map((image, index) => (
          <div
            key={image.id}
            className={`absolute transition-opacity w-full duration-500 bg-black ${currentSlide === index ? "opacity-100" : "opacity-0"
              }`}
          >
            <img
              src={image.img}
              alt={`Image ${index + 1}`}
              className="object-cover rounded w-[100%] p-0 m-0 h-[20vh] lg:h-[55vh]"
